...
- (void)buildFooterView {
    ...
    self.skipButton.titleLabel.font = [UIFont fontWithName:@"Marker Felt" size:16.f];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (page == (pageViews.count - 1) && self.swipeToExit) {
        _accessButton = [[UIButton alloc] initWithFrame:CGRectMake(self.frame.size.width / 2 - 100, self.frame.size.height / 2, 200, self.frame.size.height / 2)];
        [_accessButton setAutoresizingMask: UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin];
        _accessButton.backgroundColor = [UIColor clearColor];
        [_accessButton addTarget:self action:@selector(skipIntroduction) forControlEvents:UIControlEventTouchUpInside];
        [self addSubview:_accessButton];
        ...
        [_accessButton removeFromSuperview];
